          ...
          <div className="grid md:hidden grid-cols-1 gap-4 p-6">
               {users.length === 0 ? (
                    <div className="py-20 text-center opacity-30">
                        ...
                        <p className="font-black uppercase text-xs tracking-widest">Không có dữ liệu nhân sự</p>
                    </div>
               ) : users.map(u => (
                    <div key={u.id} className="bg-white dark:bg-zinc-800 rounded-[2.5rem] border border-zinc-100 dark:border-zinc-700/50 p-6 flex flex-col gap-4 shadow-sm active:scale-[0.98] transition-transform relative overflow-hidden group cursor-pointer">
                        <div className="flex items-center gap-4">
                             ...
                             <div className="flex-1 min-w-0">
                                ...
                                <div className="flex items-center gap-2">
                                     ...
                                     <span className={`px-1.5 py-0.5 rounded-md text-[8px] font-black uppercase ${u.role === 'Admin' ? 'bg-indigo-50 dark:bg-indigo-950 text-indigo-600' : 'bg-zinc-100 dark:bg-zinc-700 text-zinc-500'}`}>{u.role === 'Admin' ? 'Quản trị viên' : 'Nhân viên'}</span>
                                </div>
                             </div>
                        </div>
                        {u.role !== 'Admin' && (
                            <div className="flex items-center justify-end gap-3 pt-4 border-t border-zinc-50 dark:border-zinc-700/50">
                                 <button onClick={() => { setShowForm(true); setFormUser(u); setIsEdit(true); }} className="flex-1 py-3 bg-zinc-50 dark:bg-zinc-800 text-zinc-900 dark:text-zinc-100 rounded-xl text-[10px] font-black uppercase tracking-widest flex items-center justify-center gap-2 border border-zinc-100 dark:border-zinc-700 cursor-pointer"><Edit2 className="w-3.5 h-3.5" /> Chỉnh sửa</button>
                                 <button onClick={() => handleDelete(u.id)} className="flex-1 py-3 bg-rose-50 dark:bg-rose-900/20 text-rose-600 dark:text-rose-400 rounded-xl text-[10px] font-black uppercase tracking-widest flex items-center justify-center gap-2 cursor-pointer"><Trash2 className="w-3.5 h-3.5" /> Xóa</button>
                            </div>
                        )}
                    </div>
               ))}
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-zinc-50 dark:divide-zinc-800/50">
                {users.length === 0 ? (
                  ...
                  users.map((u) => (
                    <tr key={u.id} className="hover:bg-zinc-50/50 dark:hover:bg-zinc-800/50 transition-all group cursor-pointer">
                      ...
                      <td className="py-6 px-10">
                           <span className="px-3 py-1 bg-zinc-100 dark:bg-zinc-800 rounded-lg text-[10px] font-black text-zinc-500 uppercase tracking-widest">
                                {u.createdAt ? new Date(u.createdAt).toLocaleDateString() : 'Chưa rõ'}
                           </span>
                      </td>
          ...
          <div className="bg-white dark:bg-zinc-900 w-full sm:max-w-md sm:rounded-[3rem] shadow-2xl overflow-hidden flex flex-col h-full sm:h-auto max-h-[100vh] sm:max-h-[85vh] animate-in zoom-in-95" onClick={e => e.stopPropagation()}>
            ...
            <div className="p-8 pb-12">
              <form onSubmit={handleSubmit} className="space-y-6">
                 <div>
                    <label className="block mb-2 text-[10px] font-black uppercase tracking-widest text-zinc-400 ml-1">Tên đăng nhập (Username)</label>
                    <input
                      className="w-full p-4 bg-zinc-50 dark:bg-zinc-800 border-none rounded-2xl text-sm font-black focus:ring-4 focus:ring-indigo-500/10 placeholder:text-zinc-300 disabled:opacity-50 cursor-pointer"
                  ...
                  <div>
                    <label className="block mb-2 text-[10px] font-black uppercase tracking-widest text-zinc-400 ml-1">
                      {isEdit ? "Mật khẩu mới (Để trống nếu không đổi)" : "Mật khẩu (Password)"}
                    </label>
                    <input
                      type="password"
                      className="w-full p-4 bg-zinc-50 dark:bg-zinc-800 border-none rounded-2xl text-sm font-black focus:ring-4 focus:ring-indigo-500/10 placeholder:text-zinc-300 cursor-pointer"
                  ...
                  <div>
                    <label className="block mb-2 text-[10px] font-black uppercase tracking-widest text-zinc-400 ml-1">Họ và tên nhân viên</label>
                    <input
                      className="w-full p-4 bg-zinc-50 dark:bg-zinc-800 border-none rounded-2xl text-sm font-black focus:ring-4 focus:ring-indigo-500/10 placeholder:text-zinc-300 cursor-pointer"
                  ...
                  <div>
                    <label className="block mb-2 text-[10px] font-black uppercase tracking-widest text-zinc-400 ml-1">Quyền hạn truy cập</label>
                    <select
                      className="w-full p-4 bg-zinc-50 dark:bg-zinc-800 border-none rounded-2xl text-sm font-black focus:ring-4 focus:ring-indigo-500/10 dark:text-white disabled:opacity-50 appearance-none cursor-pointer"
                  ...
                  <button
                    type="submit"
                    className="w-full py-5 bg-zinc-900 dark:bg-white text-white dark:text-zinc-900 text-xs font-black rounded-2xl shadow-2xl active:scale-95 disabled:opacity-50 flex items-center justify-center gap-3 uppercase tracking-widest mt-8 cursor-pointer"
                    disabled={formLoading}
                  >
                    {formLoading ? (
                      <Loader2 className="w-5 h-5 animate-spin" />
                    ) : (
                      isEdit ? "XÁC NHẬN CẬP NHẬT" : "CẤP TÀI KHOẢN TRUY CẬP"
                    )}
                  </button>
              </form>
            </div>
